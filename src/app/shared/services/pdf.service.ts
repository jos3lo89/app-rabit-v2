...
import { PdfV2 } from '../interfaces/pdf.interfaces';
...
export class PdfService {
  ...
  async generarBoleta(
    ...
    try {
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text(`${this.ticketInfo.titulo} - ${num_ticket}`, 45, 40, {
        align: 'center',
      });
      ...
      const mesa = num_mesa !== 0 ? `Mesa N°: ${num_mesa}` : 'Para llevar';
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(`Total a pagar: S/ ${totalPagar.toFixed(2)}`, 10, y);
  ...
  async generarBoletaHistory(ticketv2: PdfV2) {
    try {
      const doc = new jsPDF({
        orientation: 'portrait',
        unit: 'mm',
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text(`${this.ticketInfo.titulo} - ${ticketv2.num_ticket}`, 45, 40, {
        align: 'center',
      });
      ...
      // let y = 90;
      let y = 76;
      ticketv2.products.forEach((item) => {
        doc.text(`${item.cantidad}`, 10, y);
        doc.text(`${item.nombre.substring(0, 15)}`, 25, y);
        doc.text(`${item.precioUnidad.toFixed(2)}`, 55, y, { align: 'right' });
        doc.text(`${item.precioTotal.toFixed(2)}`, 75, y, { align: 'right' });
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(`Total a pagar: S/ ${ticketv2.totalAmount.toFixed(2)}`, 10, y);
